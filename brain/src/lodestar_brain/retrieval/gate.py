"""The chosen architecture's one addition after retrieval: ask a model which
contexts help.
...
async def relevance_gate(llm, query: str, documents: list[Document],
    ...
    """The chosen architecture's one addition after retrieval: drop the contexts
    a model says do not help. An empty result is the honest answer — without a
    gate every question gets an answer, including the ones the board cannot
    support."""
