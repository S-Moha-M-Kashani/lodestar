...
**The direction is one-way.** `prompt` and `result` import nothing from this
package, `graph` imports both, and nothing here imports `__init__` — so the
module everything needs is the one that needs nothing. The fence itself is not
here at all: it is `middleware/untrusted.py`, next to the error handler it has
to sit outside of.
"""
